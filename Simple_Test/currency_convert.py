...
currency = (input("'Monney' 'Unit' : ")).split(" ")
unit = str(input("Choose currency to convert for : "))

euro_dinar = (int(currency[0])) * 200
dinar_euro = (int(currency[0])) / 200
dollar_dinar = (int(currency[0])) * 160
dinar_dollar = (int(currency[0])) / 160
euro_dollar = (int(currency[0])) * 1.5
dollar_euro = (int(currency[0])) / 1.5
